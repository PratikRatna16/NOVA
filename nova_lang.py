...
def web_style_node(state: NovaState) -> NovaState:
    ...
    messages = [
        ...
        HumanMessage(content=(
            ...
            "9. PRICING SECTION: Every pricing card (any class containing 'pricing-card', 'plan-card', 'price-card') MUST have: background, border, border-radius:12px, padding:2rem, display:flex, flex-direction:column. The pricing grid MUST be display:grid, grid-template-columns:repeat(3,1fr), gap:2rem.\n"
            "10. FAQ SECTION: The FAQ container MUST have display:block. Each .faq-item MUST have border-bottom. Each .faq-question MUST have display:flex, justify-content:space-between, cursor:pointer, padding:1.5rem. Each .faq-answer MUST have display:none, padding:0 1.5rem 1.5rem — JS will toggle it open.\n"
            "11. TESTIMONIALS SECTION: The testimonials wrapper MUST have overflow:hidden. The track (any class containing 'track') MUST have display:flex, transition:transform 0.3s ease. Each testimonial card MUST have min-width:100%, padding:2rem, background, border-radius:12px.\n"
            "12. PRIORITY ORDER: Generate CSS in this order — reset/variables, nav, hero, features, PRICING, FAQ, TESTIMONIALS, footer, responsive. Never skip pricing, FAQ, or testimonials even if output is long.\n"
            "9. PRICING SECTION — MANDATORY: Any class containing 'pricing-card' or 'plan-card' MUST have: background, border, border-radius:12px, padding:2rem, display:flex, flex-direction:column. The pricing grid MUST use display:grid with repeat(3,1fr). Featured/highlighted card MUST have a distinct background or border color.\n"
            "10. FAQ SECTION — MANDATORY: Any class containing 'faq-item' or 'accordion-item' MUST have: border-bottom, padding. Any class containing 'faq-question' or 'accordion-header' MUST have: display:flex, justify-content:space-between, cursor:pointer, padding. Any class containing 'faq-answer' or 'accordion-content' MUST have: display:none, padding (JS toggles to display:block). Add a CSS + or ▼ indicator via ::after pseudo-element.\n"
            "11. TESTIMONIALS SECTION — MANDATORY: Any class containing 'testimonial-card' MUST have: background, border-radius:12px, padding:2rem. The carousel wrapper MUST have: overflow:hidden. The track MUST have: display:flex, transition:transform 0.3s ease. Navigation buttons MUST have: background, border-radius:50%, cursor:pointer, padding.\n"
            "12. WRITE CSS IN THIS ORDER: (1) reset/variables, (2) navbar, (3) hero, (4) features, (5) pricing, (6) FAQ, (7) testimonials, (8) footer, (9) responsive. Never stop before reaching footer and responsive rules.\n"
        ))
    ]
